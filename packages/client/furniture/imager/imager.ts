import FurniBase from './base'
import FurniAsset from './asset'
import { 
	Furnidata, 	
	Size,
	ItemType,
...
export default class FurniImager {
    ready: boolean
    bases: /* { roomitem: { [id: string]: Promise<FurniBase> }, wallitem: { [id: string]: Promise<FurniBase> } } */ any
    offsets: { [id: string]: Promise<Offset> };
    furnidata: Furnidata;
    ...
    findItemById(type: ItemType, itemId: number): Description | null {
        const furnidataType = type === 'floor' ? 'roomitemtypes' : 'wallitemtypes';
    ...
    _fetchOffsetAsync(uniqueName: string): Promise<Offset> {
        return new Promise((resolve, reject) => {
            this._fetchJsonAsync(LOCAL_RESOURCES_URL + uniqueName + '/furni.json').then(data => {
                resolve(data as Offset);
            }).catch(err => reject(err));
        });
    }
}
