...
# def color_convert(n): return [int(i)
#                               for i in n.to_bytes(4, byteorder='big', signed=True)]
...
#         loc = GuildBadge.calculate_drawing_point(
#             badge.size, self.__base.image_data.shape, self.__points[0])
#         badge.alpha_composite(Image.fromarray(
#             (self.__base.image_data * self.__colors[0]).astype(np.uint8)), (int(loc[0]), int(loc[1])))
#         for i in range(len(self.__symbols)):
#             loc = GuildBadge.calculate_drawing_point(
#                 badge.size, self.__symbols[i].image_data.shape, self.__points[i+1])
#             badge.alpha_composite(Image.fromarray(
#                 (self.__symbols[i].image_data * self.__colors[i+1]).astype(np.uint8)), (int(loc[0]), int(loc[1])))
#             if self.__symbols[i].two_parts:
#                 badge.alpha_composite(Image.fromarray(
#                     self.__symbols[i].image_data_2), (int(loc[0]), int(loc[1])))
#         return badge
...
#                     self.__base_colors[int(line[0])] = np.append(
#                         np.asarray(color_convert(int(line[1], 16))[1:5]) / 255, 1)

#                 if line[3] == "symbol_color":
#                     self.__symbol_colors[int(line[0])] = np.append(
#                         np.asarray(color_convert(int(line[1], 16))[1:5]) / 255, 1)

#                 if line[3] == "base":
#                     base = BadgePart(int(line[0]), BadgePartTypes.BASE, (np.asarray(Image.open('./resources/badge/badgepart_' + line[1] + '.png'))) if line[1]
#                                      is not '' else None, (np.asarray(Image.open('./resources/badge/badgepart_' + line[2] + '.png'))) if line[2] is not '' else None)
#                     self.__bases[base.id] = base

#                 if line[3] == "symbol":
#                     symbol = BadgePart(int(line[0]), BadgePartTypes.SYMBOL, (np.asarray(Image.open(('./resources/badge/badgepart_' + line[1] + '.png')).convert("RGBA"))) if line[1]
#                                        is not '' else None, (np.asarray(Image.open(('./resources/badge/badgepart_' + line[2] + '.png')).convert("RGBA"))) if line[2] is not '' else None)
...
#                 badge.set_base(data_loader.get_base(
#                     id), data_loader.get_base_color(color), pos)
#             elif type == BadgePartTypes.SYMBOL.value:
#                 badge.add_symbol(data_loader.get_symbol(
#                     id), data_loader.get_symbol_color(color), pos)
