...
export default class Messenger extends React.Component<MessengerProps, MessengerState> {
    ...
    generateRequestsGrid(friends: User[]): React.ReactNode {
        return friends.map(friend => {
            return (
                <div key={friend.id} className="friend">
                    {this.generateAvatarImage(friend.look, friend.name)}
    ...
    generateOnlineFriendsGrid(friends: User[]): React.ReactNode {
        return friends.map(friend => {
            return (
                <div key={friend.id} className="friend">
                    {this.generateAvatarImage(friend.look, friend.name)}
    ...
    generateOfflineFriendsGrid(friends: User[]): React.ReactNode {
        return friends.map(friend => {
            return (
                <div key={friend.id} className="friend">
                    <span>{friend.name}</span>
    ...
    generateSearchGrid(friends: User[]): React.ReactNode {
        return friends.map(friend => {
            return (
                <div key={friend.id} className="friend">
                    {this.generateAvatarImage(friend.look, friend.name)}
    ...
    generateAvatarImage(figure: string, friendName: string): React.ReactNode {
        ...
        avatarImager.generateGeneric(new AvatarInfo(figure, 2, 2, ["std"], "std", 0, true, false, "s"), false).then(canvas => {
