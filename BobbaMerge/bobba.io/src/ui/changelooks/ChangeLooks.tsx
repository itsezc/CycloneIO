...
class ChangeLooks extends React.Component<ChangeLooksProps, ChangeLooksState>  {
    ...
    generatePalette(paletteId: number): React.ReactNode {
    ...
    generatePalettes(): React.ReactNode {
        const currentPart = this.calculateCurrentPart();
        const palettes: React.ReactNode[] = [];
        let colorCount = 1;
        if (currentPart != null) {
    ...
    generatePartImage(figure: string, selected: boolean, type: MainTabId): React.ReactNode {
        const ref: React.RefObject<HTMLImageElement> = React.createRef();
        const avatarImager = BobbaEnvironment.getGame().avatarImager;

        avatarImager.generateGeneric(new AvatarInfo(figure, 2, 2, ["std"], "std", 0, type === 'generic', false, "n"), false).then(canvas => {
    ...
    generatePlatform(): React.ReactNode {
