...
export default class Catalogue extends React.Component<CatalogueProps, CatalogueState> {
    ...
    generateDescription(): React.ReactNode {
                ...
                return (
                    <>
                        ...
                        <div className="button_container">
                            <span>{item.cost} crédits{item.cost === 1 ? '' : 's'}</span>
            ...
            if (item.baseItem == null) {
                ...
                return (
                    <button key={item.itemId} onClick={this.handleSelectItem(item.itemId)} className={currentItemId === item.itemId ? 'selected' : ''}>
                        <img src={ (image) ? image.src : ''} alt={item.baseItem.furniBase.itemData.name} />
                    </button>
                );
            }
        });
    ...
    generateFrontPage(currentPage: CataloguePage): React.ReactNode {
        return (
            <>
    ...
    generateDefaultPage(currentPage: CataloguePage): React.ReactNode {
        return (
            <>
