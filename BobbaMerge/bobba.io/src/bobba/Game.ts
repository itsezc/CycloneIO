...
export default class Game {
    ...
    constructor() {
        this.engine = new MainEngine(this.gameLoop, this.onResize, this.onMouseMove, this.onTouchStart, this.onTouchMove, this.onMouseClick, this.onMouseDown, this.onMouseDoubleClick);
    ...
    async loadGame(): Promise<void> {
        ...
        await this.avatarImager.initialize().then(() => this.ghostTextures.initialize());
        await this.furniImager.initialize();
        await this.chatImager.initialize();
        await this.meMenuImager.initialize();
        await this.roomImager.initialize();
        await this.engine.loadGlobalTextures(sprites);
    ...
    onMouseDown = (x: number, y: number, altKey: boolean) => {
        if (this.currentRoom != null) {
            this.currentRoom.engine.handleMouseDown(x, y, altKey);
        }
    }
