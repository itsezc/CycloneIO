...
export default class ChatImager {
    ...
    async initialize(): Promise<void> {
        ...
        const style = await this._downloadChatStyle(regpoints, base, pointer, color);
        this.chatStyles[0] = style;
    }

    async _downloadChatStyle(regPoints: string, base: string, pointer: string, color: string): Promise<ChatStyle> {
        const regPointsData = await this._fetchJsonAsync(regPoints);
        const baseData = await this._downloadImageAsync(base);
        const pointerData = await this._downloadImageAsync(pointer);
        const colorData = await this._downloadImageAsync(color);
        return new ChatStyle((regPointsData as RegPoints), baseData, pointerData, colorData);
    }
