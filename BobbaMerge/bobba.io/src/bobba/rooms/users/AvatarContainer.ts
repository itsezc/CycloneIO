...
export default class AvatarContainer {
    ...
    async _loadUniqueBodyTexture(direction: Direction, action: string[], frame: number): Promise<void> {
        ...
        const image = await avatarImager.generateGeneric(new AvatarInfo(this.look, direction, direction, action, "std", frame, false, true, "n"), this.isGhost);
    ...
    async _loadUniqueHeadTexture(headDirection: Direction, gesture: string, frame: number): Promise<void> {
        ...
        const image = await avatarImager.generateGeneric(new AvatarInfo(this.look, headDirection, headDirection, ["std"], gesture, frame, true, false, "n"), this.isGhost);
    ...
    async _loadChatHeadImage(): Promise<void> {
        ...
        const canvas = await avatarImager.generateGeneric(new AvatarInfo(this.look, 2, 2, ["std"], "std", 0, true, false, "s"), this.isGhost);
    ...
    async _loadUserInfoImage(): Promise<void> {
        ...
        const canvas = await avatarImager.generateGeneric(new AvatarInfo(this.look, 4, 4, ["std"], "std", 0, false, false, "n"), this.isGhost);
