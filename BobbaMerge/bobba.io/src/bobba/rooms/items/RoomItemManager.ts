...
export default class RoomItemManager {
    ...
    async addFloorItemToRoom(id: number, x: number, y: number, z: number, rot: Direction, state: number, baseId: number): Promise<RoomItem> {
        const item = this.getItem(id);
        if (item != null) {
        ...
        const containerGroup = await newItem.loadBase();
    ...
    async addWallItemToRoom(id: number, x: number, y: number, rot: Direction, state: number, baseId: number): Promise<RoomItem> {
        const item = this.getItem(id);
        if (item != null) {
            this.removeItemFromRoom(id, false);
        }
