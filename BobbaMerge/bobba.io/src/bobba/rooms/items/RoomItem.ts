...
export default abstract class RoomItem implements Selectable {
    ...
    setAdditionalSprites() {
        if (this.baseItem != null) {
            const layerCount = this.baseItem.furniBase.offset.visualization.layerCount + 1;
    ...
    updateTextures() {
        if (this.baseItem != null) {
            ...
            if (this.drawAsIcon) {
                ...
                container.zIndex = this.calculateZIndex(0, 0);
                layerIndex++;
            } else {
                ...
                for (let layer of this.baseItem.furniBase.getLayers(this.rot, actualState, actualFrame)) {
                    ...
                    if (texture != null) {
                        ...
                        this.containers[layerIndex].zIndex = this.calculateZIndex(zIndex, layer.id);
    ...
    async loadBase(): Promise<ContainerGroup> {
        this.baseItem = await BobbaEnvironment.getGame().baseItemManager.getItem(this.getItemType(), this.baseId);
    ...
    showItemInfo(isUpdate: boolean) {
        if (this.baseItem != null) {
            BobbaEnvironment.getGame().uiManager.onSelectFurni(this.id, this.baseId, this.baseItem.furniBase.itemData.name, this.baseItem.furniBase.itemData.description, this.baseItem.infoImage, isUpdate);
        }
    }
